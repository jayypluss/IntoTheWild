...
@onready var blueprint_placement_holdoff: Timer = $BlueprintPlacementHoldoff
		...
		if (is_instance_valid(get_child(1))): 
			...
			if blueprint:
				...
				if !collider:
					...
					if Input.is_action_just_pressed('click') and blueprint_placement_holdoff.is_stopped():
						place_blueprint()
